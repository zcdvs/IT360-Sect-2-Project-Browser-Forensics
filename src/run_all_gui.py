...
# Since all scripts are in src/, ROOT_DIR is now the src folder itself
SRC_DIR = Path(__file__).resolve().parent
ROOT_DIR = SRC_DIR.parent  # Project root (for outputs folder)
RUN_ALL_SCRIPT = SRC_DIR / 'run_all.py'
...
class RunAllGUI(tk.Tk):
    ...
    def _do_run_all(self):
        ...
        self.runner = RunnerThread(cmd, cwd=str(SRC_DIR), log_callback=self._log, done_callback=self._on_run_done)
        self.runner.start()
